...
(() => {
  ...
  // Versão estática (GitHub Pages): sem servidor, dados vêm de arquivos JSON
  // e os formulários enviam pelo WhatsApp.
  const STATIC = root.hasAttribute('data-static');
  const BASE = root.dataset.base || '/';
  const url = (p) => (p && p.startsWith('/') ? BASE + p.slice(1) : p);
  const whatsapp = (text) => {
    const wa = (document.querySelector('.wa-float')?.getAttribute('href') || 'https://wa.me/').split('?')[0];
  ...
  if (bp) {
    ...
    fetch(STATIC ? BASE + 'data/posts.json' : '/api/posts?limit=3')
      .then((r) => r.ok ? r.json() : Promise.reject())
      .then(({ items }) => {
        if (!items.length) { $('#blog').remove(); return; }
        bp.innerHTML = items.map((p, i) => `
          <article class="post-card reveal" style="--rd:${i * 0.1}s">
            <a href="${url('/blog/' + p.slug)}" class="post-card__media" tabindex="-1" aria-hidden="true">
              ${p.cover ? `<img src="${esc(url(p.cover))}" alt="" loading="lazy">` : `<div class="post-card__ph"><svg viewBox="0 0 64 40" aria-hidden="true"><path d="M4 32l14-14 10 8 18-20" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><circle cx="50" cy="6" r="4" fill="#bef264"/></svg><em>${esc(p.category || 'Blog')}</em></div>`}
            </a>
            <div class="post-card__body">
              <div class="post-meta">${p.category ? `<a class="chip" href="${url('/blog/categoria/' + p.category_slug)}">${esc(p.category)}</a>` : ''}<span>${date(p.published_at)}</span><span>·</span><span>${p.reading_time} min</span></div>
              <h3><a href="${url('/blog/' + p.slug)}">${esc(p.title)}</a></h3>
              <p>${esc(p.excerpt)}</p>
              <a href="${url('/blog/' + p.slug)}" class="link-arrow">Ler artigo <span aria-hidden="true">→</span></a>
            </div>
          </article>`).join('');
  ...
  if (form) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (STATIC) {
        whatsapp(`Olá! Quero uma demonstração.
Nome: ${data.name}
Laboratório: ${data.company || '-'}
E-mail: ${data.email}
WhatsApp: ${data.phone || '-'}
${data.message || ''}`);
        form.reset(); msg.className = 'form__msg is-ok'; msg.textContent = '✓ Abrimos o WhatsApp com sua mensagem. É só enviar!';
        return;
      }
  ...
  if (grid) {
    fetch(STATIC ? BASE + 'data/testimonials.json' : '/api/testimonials').then((r) => r.ok ? r.json() : Promise.reject()).then(({ items, total, avg }) => {
  ...
  if (rm) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (STATIC) {
        whatsapp(`Meu depoimento (${d.rating}★)
${d.name}${d.role ? ' — ' + d.role : ''}${d.company ? ' · ' + d.company : ''}

${d.text}`);
        form.reset(); cnt.textContent = '0/600'; $('#rateLabel').textContent = 'Toque nas estrelas';
        form.hidden = true; okBox.hidden = false; $('button', okBox).focus();
        return;
      }
